...
function getAcademicWeekNumber(date: Date) {
  ...
  const startWeek = getAcademicWeekNumber(start);

  // 🔒 If range ≤ 7 days → single occurrence
  if ((end.getTime() - start.getTime()) <= 7 * 24 * 60 * 60 * 1000) {
    return [startWeek];
  }

  // Otherwise weekly recurrence
  const weeks: number[] = [];
  ...
  return Array.from(new Set(weeks));
    ...
    function jsDayToBackendDay(jsDay: number) {
  ...
  return jsDay + 1; // Mon=2 ... Sat=7
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const selectedCourse = courses.find(c => c._id === selectedCourseId);
           ...
           const jsDay = new Date(dateStart + "T00:00:00").getDay();
            const backendDay = jsDayToBackendDay(jsDay);

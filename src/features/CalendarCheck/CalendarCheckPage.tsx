...
export default function CalendarCheckPage() {
  ...
  const filtered = useMemo(() => {
    ...
    //   result = result.filter((c: any) => c.teacherId === savedUser.id);
    // }

    // if (savedUser.role === 'student') {
    //   result = result.filter((c: any) => c.studentIds?.includes?.(savedUser.id));
    // }
    ...
    if (q) {
      result = result.filter(
        (c) =>
          ...
          c.sessions.some(
            (s) => (s.room ?? '').toLowerCase().includes(q) || String(s.weeks).includes(q)
          )
      );
    }
              ...
              <table className="w-full table-auto text-sm">
                <thead>
                  <tr className="bg-blue-600 text-white">
                    ...
                    <th className="px-3 py-2 text-left">This Week</th>
                  </tr>
                </thead>
                ...
                <div className="grid grid-cols-7 border-l border-t" style={{ minWidth: 800 }}>
                  ...
                  {weekDates.map((_, dayIdx) => (
                    <div
                      ...
                      style={{ minHeight: HOURS.length * 40 }}>
